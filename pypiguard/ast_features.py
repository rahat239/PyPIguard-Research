...
# NOTE: __import__ and execfile were deliberately removed from this set after
# discovering __import__(package) is a common, benign dynamic-import idiom
# (found even in the `requests` library itself) that caused false positives
# when treated as equally suspicious to eval/exec. The __import__(...).method()
# REDIRECTION pattern (e.g. __import__('os').system(...)) remains separately
# and correctly detected via ast_num_getattr_obfuscation below, since chaining
# a dangerous module method call onto a dynamic import is a much stronger,
# more specific signal than a bare __import__() call alone.
