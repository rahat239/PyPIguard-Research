...
- The final integrated dataset skews toward smaller packages; two real-world false-verdict cases were identified and corrected during post-deployment testing (detailed under Deployment Validity below), and packages far larger than anything in the training distribution remain a documented, flagged limitation rather than a silently unreliable one.

**Deployment validity** (issues surfaced only after deploying the system as a real, publicly-usable tool, rather than through offline evaluation alone):
- During live deployment testing, the `requests` library -- one of the most widely trusted packages in the Python ecosystem -- was initially misclassified with a near-50% malicious score. Root-cause analysis identified an overly broad feature definition: `__import__()` calls were grouped with `eval`/`exec`/`compile` as equally "dangerous," despite `__import__(package)` being a common, benign dynamic-import idiom present in `requests`' own source. We corrected the feature definition, re-extracted AST features for all 9,721 training samples, and retrained the final model; held-out ROC-AUC changed negligibly (0.9993 to 0.9991), while the false positive was resolved. We report this as evidence that offline validation metrics, however extensive, do not substitute for testing a deployed system against real, familiar software.
- The same deployment testing revealed a second, more severe issue: `numpy` (8,204 files) was misclassified as malicious, because its size exceeds the maximum package size seen anywhere in training (5,374 files) -- a genuine out-of-distribution input, not merely a low-confidence prediction. Rather than retraining again under time constraints or silently accepting an unreliable verdict, we added an explicit out-of-distribution safety gate to the deployed tool: packages exceeding 1.2x the largest training sample size are flagged as "unreliable" in the interface rather than issued a confident-looking but untrustworthy CLEARED/FLAGGED verdict. We consider this the more honest engineering response to a real distributional limitation than either ignoring it or over-fitting a fix under time pressure.
- The public web deployment (hosted on a free-tier platform) imposes an approximately 30-second request timeout, which can be exceeded when live-scanning unusually large packages (confirmed directly: a live `numpy` scan took 32 seconds and was terminated by the hosting proxy with an HTTP 502). This is an infrastructure constraint of the deployment tier, not a limitation of the underlying detection method; the notebook-based batch pipeline and self-hosted deployment are both unaffected.

---

## VI. Conclusion and Future Work

This paper presented a lightweight, explainable, LLM-free machine learning approach for detecting malicious PyPI packages, validated through an extensive battery of experiments spanning baseline comparison, held-out cross-dataset validation, metadata-vs-behavioral feature ablation, multi-seed stability analysis (re-confirmed on the final model), causal feature ablation, statistical significance testing, bootstrapped confidence intervals, dataset scale-up to the full available malicious archive (9,723 samples), cross-ecosystem generalization testing (re-confirmed stable across dataset scales), adversarial evasion robustness testing with AST- and bytecode-level mitigation (reducing evasion susceptibility from 9% to 2%), temporal validation against evolving attacker techniques (0.984 ROC-AUC on a strict time-based split), class-imbalance sensitivity analysis, inference latency benchmarking, and a labeled matched comparison against a Cerebro-inspired baseline. Our final integrated model achieves 0.9993 ROC-AUC on genuinely unseen, leakage-verified data, independently reproduced end-to-end across two separate environments with matching results to four decimal places. We report our results with explicit honesty about remaining limitations, formalized as Internal, External, Construct, and Deployment validity threats, including partial cross-ecosystem transfer, static-only analysis by deliberate scope decision, a matched comparison in which a simplified sequence-based baseline modestly outperformed our approach, and two real false-verdict cases identified and corrected during live deployment testing -- underscoring that deployment as a real, publicly-usable tool surfaces failure modes that offline evaluation alone does not. Future work should combine our fast tabular features with sequence-order modeling, extend static analysis with dedicated, properly-isolated dynamic/behavioral analysis infrastructure, and pursue a more naturally-balanced large-scale benign corpus.
